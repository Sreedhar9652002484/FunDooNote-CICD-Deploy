    ...
    public class NotesController : Controller
    {
       ...
       /* [Authorize]
        ...
        public IActionResult AddColour(long NotesId, string Colour)
        {
            ...
            if (userId != null)
            {
                var result = notesBusiness.Colour(NotesId, Colour, userId);
                if (result != null)
                {
        ...
        public async Task<IActionResult> AddImage(long notedId, string item,  IFormFile imageFile)
        {
            try
            {
                ...
                if (userId != null)
                {
                    ...
                    var result = await notesBusiness.AddImage(notedId, userId, item, imageFile);
        ...
        [Authorize]
        [Route("FindNotesUsingKeyword")]
        [HttpGet]
        public IActionResult findNotes(string keyword)
        {
            long userId = Convert.ToInt64(User.Claims.FirstOrDefault(x => x.Type == "UserId").Value);
            if (userId != null)
            {
                var result = notesBusiness.FindNotes(keyword, userId);
                if (result != null)
                {
                    return this.Ok(new { success = true, Message = " Notes Retrived Successful", data = result });
                }
                else
                {
                    return this.BadRequest(new { success = false, Message = "UnSuccessful", Data = result });
        ...
        [Route("CopyNotes")]
        [HttpPost]
        public IActionResult CopyNotes(long notesId)
        {
            long userId = Convert.ToInt64(User.Claims.FirstOrDefault(x => x.Type == "UserId").Value);
            if (userId != null)
            {
                var result = notesBusiness.CopyNotes(userId, notesId);
                if (result != null)
                {
                    return this.Ok(new { success = true, Message = " Notes Copied Successful", data = result });
                }
                else
                {
                    return this.BadRequest(new { success = false, Message = "UnSuccessful", Data = result });
